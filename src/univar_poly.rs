...
pub struct UnivarPolynomial(pub CurveOrderElementVector);

impl UnivarPolynomial {
    /// Return a zero polynomial of degree `degree`
    pub fn new(degree: usize) -> Self {
        let coeffs = CurveOrderElementVector::new(degree + 1);
        UnivarPolynomial(coeffs)
    }

    /// Return a constant polynomial
    pub fn new_constant(constant: CurveOrderElement) -> Self {
        let mut coeffs = CurveOrderElementVector::new(1);
        coeffs[0] = constant;
        UnivarPolynomial(coeffs)
    }
    ...
    pub fn random(degree: usize) -> Self {
        Self(CurveOrderElementVector::random(degree + 1)) // +1 for constant term
    }

    /// Create a polynomial with given roots in `roots`
    /// i.e. (x-roots[0])*(x-roots[1])*(x-roots[2])...(x-roots[last]) given `roots`
    pub fn new_with_roots(roots: &[CurveOrderElement]) -> Self {
        // vector of [(x-roots[0]), (x-roots[1]), (x-roots[2]), ...]
        let x_i = roots
            .par_iter()
            .map(|i| {
                let mut v = CurveOrderElementVector::with_capacity(2);
                v.push(-i);
        ...
        x_i.par_iter().cloned().reduce(
            || Self::new_constant(CurveOrderElement::one()),
            |a, b| UnivarPolynomial::multiply(&a, &b),
        )
    }

    pub fn coefficients(&self) -> &CurveOrderElementVector {
        &self.0
    }
    ...
    pub fn eval(&self, x: &CurveOrderElement) -> CurveOrderElement {
        if x.is_zero() {
            self[0].clone()
        } else {
    ...
    pub fn long_division(dividend: &Self, divisor: &Self) -> (Self, Self) {
        ...
        for i in (div_degree..=rem_degree).rev() {
            if remainder[i].is_zero() {
                quotient.push(CurveOrderElement::zero());
                continue;
            }
        ...
        (
            UnivarPolynomial(CurveOrderElementVector::from(quotient)),
            remainder,
        )
    }
    ...
    pub fn multiply_by_constant(&self, constant: &CurveOrderElement) -> UnivarPolynomial {
    ...
    pub fn multiply_by_monic_monomial(&self, monomial_degree: u64) -> UnivarPolynomial {
        let mut new_poly = self.clone();
        let new_poly_beginning = CurveOrderElementVector::new(monomial_degree as usize);
    ...
    fn replace_small_degree_terms<I: IntoIterator<Item = CurveOrderElement>>(
        &mut self,
        till_degree: usize,
        replace_with: I,
    ) {
...
mod tests {
    ...
    fn test_poly() {
        let degree = 10;
        let poly1 = UnivarPolynomial(CurveOrderElementVector::random(degree + 1));
        assert!(!poly1.is_zero());

        let poly2 = UnivarPolynomial(CurveOrderElementVector::new(degree + 1));
        assert!(poly2.is_zero());

        let poly3 = UnivarPolynomial::new(degree);
        assert!(poly3.is_zero());

        let poly4 = UnivarPolynomial::new_constant(CurveOrderElement::from(100u64));
        assert!(!poly4.is_zero());
        assert_eq!(poly4.degree(), 0);
        assert_eq!(poly4[0], CurveOrderElement::from(100u64));
    }
    ...
    fn test_create_poly_from_macro() {
        let poly = univar_polynomial!(
            ...
            CurveOrderElement::from(87u64),
            -CurveOrderElement::one(),
            CurveOrderElement::from(300u64)
        );
        assert_eq!(poly.degree(), 4);
        assert_eq!(poly[0], CurveOrderElement::one());
        assert_eq!(poly[1], CurveOrderElement::zero());
        assert_eq!(poly[2], CurveOrderElement::from(87u64));
        assert_eq!(poly[3], CurveOrderElement::minus_one());
        assert_eq!(poly[4], CurveOrderElement::from(300u64));
    }
    ...
    fn test_poly_long_div() {
        ...
        let dividend = UnivarPolynomial(CurveOrderElementVector::from(c1));
        // divisor = 1 + x
        let c2 = vec![CurveOrderElement::one(), CurveOrderElement::one()];
        let divisor = UnivarPolynomial(CurveOrderElementVector::from(c2));
        let (quotient, rem) = UnivarPolynomial::long_division(&dividend, &divisor);
        println!("Quotient={:?}", &quotient);
        // quotient = -1 + x
        assert_eq!(quotient.degree(), 1);
        assert_eq!(quotient[0], CurveOrderElement::minus_one());
        assert_eq!(quotient[1], CurveOrderElement::one());
        ...
        // quotient = 1 + x
        assert_eq!(quotient.degree(), 1);
        assert_eq!(quotient[0], CurveOrderElement::one());
        assert_eq!(quotient[1], CurveOrderElement::one());
        ...
        let c1 = vec![
            CurveOrderElement::one(),
            CurveOrderElement::from(3u64),
            CurveOrderElement::from(2u64),
        ];
        let dividend = UnivarPolynomial(CurveOrderElementVector::from(c1));
        // divisor = 1 + x
        let c2 = vec![CurveOrderElement::one(), CurveOrderElement::one()];
        ...
        assert_eq!(quotient.degree(), 1);
        assert_eq!(quotient[0], CurveOrderElement::one());
        assert_eq!(quotient[1], CurveOrderElement::from(2u64));

        assert_eq!(rem.degree(), 0);

        // 4x - 4 / x - 1 = 4
        // dividend = -4 + 4x
        let c1 = vec![-CurveOrderElement::from(4u64), CurveOrderElement::from(4u64)];
        let dividend = UnivarPolynomial(CurveOrderElementVector::from(c1));
        // divisor = -1 + x
        let c2 = vec![CurveOrderElement::minus_one(), CurveOrderElement::one()];
        let divisor = UnivarPolynomial(CurveOrderElementVector::from(c2));
        let (quotient, rem) = UnivarPolynomial::long_division(&dividend, &divisor);
        println!("Quotient={:?}", &quotient);
        ...
        assert_eq!(quotient[0], CurveOrderElement::from(4u64));
        ...
        assert_eq!(quotient[0], CurveOrderElement::from(4u64));
        assert_eq!(quotient[1], CurveOrderElement::zero());
        assert_eq!(quotient[2], CurveOrderElement::zero());
        assert_eq!(quotient[3], CurveOrderElement::one());
        ...
        let c1 = vec![
            -CurveOrderElement::from(80u64),
            -CurveOrderElement::from(56u64),
            CurveOrderElement::from(3u64),
            -CurveOrderElement::from(40u64),
            CurveOrderElement::from(2u64),
        ];
        let dividend = UnivarPolynomial(CurveOrderElementVector::from(c1));
        // divisor = -20 + x
        let c2 = vec![-CurveOrderElement::from(20), CurveOrderElement::one()];
        let divisor = UnivarPolynomial(CurveOrderElementVector::from(c2));
        let (quotient, rem) = UnivarPolynomial::long_division(&dividend, &divisor);
        println!("Quotient={:?}", &quotient);
        ...
        assert_eq!(quotient.degree(), 3);
        assert_eq!(quotient[0], CurveOrderElement::from(4u64));
        assert_eq!(quotient[1], CurveOrderElement::from(3u64));
        assert_eq!(quotient[2], CurveOrderElement::zero());
        assert_eq!(quotient[3], CurveOrderElement::from(2u64));
    ...
    fn test_poly_multiply() {
        // (x + 1) * (x - 1) = x^2 - 1
        // x + 1
        let left = UnivarPolynomial(CurveOrderElementVector::from(vec![
            CurveOrderElement::one(),
            CurveOrderElement::one(),
        ]));
        // -1 + x
        let right = UnivarPolynomial(CurveOrderElementVector::from(vec![
            CurveOrderElement::minus_one(),
            CurveOrderElement::one(),
        ]));
        ...
        assert_eq!(product[0], CurveOrderElement::minus_one());
        assert_eq!(product[1], CurveOrderElement::zero());
        assert_eq!(product[2], CurveOrderElement::one());
        ...
        // product = 2x^2 + 3x + 1
        assert_eq!(product.degree(), 2);
        assert_eq!(product[0], CurveOrderElement::one());
        assert_eq!(product[1], CurveOrderElement::from(3u64));
        assert_eq!(product[2], CurveOrderElement::from(2u64));
        ...
        assert_eq!(product[0], CurveOrderElement::from(4u64));
        assert_eq!(product[1], CurveOrderElement::zero());
        assert_eq!(product[2], CurveOrderElement::from(4u64));
        assert_eq!(product[3], CurveOrderElement::one());
        assert_eq!(product[4], CurveOrderElement::zero());
        assert_eq!(product[5], CurveOrderElement::one());
    ...
    fn test_poly_rem() {
        ...
        assert_eq!(remainder[0], -CurveOrderElement::from(4u64));
        ...
        assert_eq!(remainder[0], CurveOrderElement::zero());
        assert_eq!(remainder[1], CurveOrderElement::minus_one());
    }
    ...
    fn test_poly_from_given_roots() {
        ...
        for _ in 0..num_test_cases {
            ...
            let roots = CurveOrderElementVector::random(num_roots);
            let poly = UnivarPolynomial::new_with_roots(roots.as_slice());
            assert_eq!(poly.degree(), num_roots);
            for r in roots {
                assert_eq!(poly.eval(&r), CurveOrderElement::zero())
            }
        }
    ...
    fn test_multiply_with_constant() {
        // 9 + 2x + 75x^2 + 128x^3
        let orig = UnivarPolynomial(CurveOrderElementVector::from(vec![
            CurveOrderElement::from(9u64),
            CurveOrderElement::from(2u64),
            CurveOrderElement::from(75u64),
            CurveOrderElement::from(128u64),
        ]));
        let c = CurveOrderElement::from(3u64);
        let new = orig.multiply_by_constant(&c);
        assert_eq!(new.degree(), 3);
        assert_eq!(new[0], CurveOrderElement::from(27));
        assert_eq!(new[1], CurveOrderElement::from(6));
        assert_eq!(new[2], CurveOrderElement::from(225));
        assert_eq!(new[3], CurveOrderElement::from(384));
        ...
        let orig = UnivarPolynomial(CurveOrderElementVector::from(vec![
            ...
            CurveOrderElement::zero(),
            CurveOrderElement::zero(),
            CurveOrderElement::from(18u64),
        ]));
        let c = CurveOrderElement::from(10u64);
        let new = orig.multiply_by_constant(&c);
        assert_eq!(new.degree(), 6);
        assert_eq!(new[0], CurveOrderElement::from(10));
        assert_eq!(new[1], CurveOrderElement::zero());
        assert_eq!(new[2], CurveOrderElement::from(40));
        assert_eq!(new[3], CurveOrderElement::from(50));
        assert_eq!(new[4], CurveOrderElement::zero());
        assert_eq!(new[5], CurveOrderElement::zero());
        assert_eq!(new[6], CurveOrderElement::from(180));
    ...
    fn test_multiply_with_monic_monomial() {
        ...
        assert_eq!(new[0], CurveOrderElement::zero());
        assert_eq!(new[1], CurveOrderElement::from(9u64));
        assert_eq!(new[2], CurveOrderElement::from(2u64));
        assert_eq!(new[3], CurveOrderElement::from(75u64));
        assert_eq!(new[4], CurveOrderElement::from(128u64));
        ...
        assert_eq!(new[0], CurveOrderElement::zero());
        assert_eq!(new[1], CurveOrderElement::zero());
        assert_eq!(new[2], CurveOrderElement::from(9u64));
        assert_eq!(new[3], CurveOrderElement::from(2u64));
        assert_eq!(new[4], CurveOrderElement::from(75u64));
        assert_eq!(new[5], CurveOrderElement::from(128u64));
        ...
        assert_eq!(new[0], CurveOrderElement::zero());
        assert_eq!(new[1], CurveOrderElement::one());
        assert_eq!(new[2], CurveOrderElement::zero());
        assert_eq!(new[3], CurveOrderElement::from(4));
        assert_eq!(new[4], CurveOrderElement::from(5));
        assert_eq!(new[5], CurveOrderElement::zero());
        assert_eq!(new[6], CurveOrderElement::zero());
        assert_eq!(new[7], CurveOrderElement::from(18));
        ...
        assert_eq!(new[0], CurveOrderElement::zero());
        assert_eq!(new[1], CurveOrderElement::zero());
        assert_eq!(new[2], CurveOrderElement::one());
        assert_eq!(new[3], CurveOrderElement::zero());
        assert_eq!(new[4], CurveOrderElement::from(4));
        assert_eq!(new[5], CurveOrderElement::from(5));
        assert_eq!(new[6], CurveOrderElement::zero());
        assert_eq!(new[7], CurveOrderElement::zero());
        assert_eq!(new[8], CurveOrderElement::from(18));
    }
}
