...
1. Map an arbitrary size message to a field element or group element by hashing the message internally.
    ```
...
1. Create vectors of field elements and do some operations
    ```rust
...
1. Create vectors of group elements and do some operations
    ```rust
    // creates a vector of size 10 with all elements as 0
...
1. Pairing support. Ate pairing is supported with target group `GT`  
    ```rust
    let g1 = G1::random();
    let g2 = G2::random();
...
1. Hash to curve point
   ...
   ```
   Check the documentation of `hash_to_curve` for more details
